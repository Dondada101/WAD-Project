...
<header>
    ...
    <div class="content">
        <?php if(isset($user)): ?>
            ...
            <?php else: ?>
                <p>Try logging in or signing up :Click Here -> <a href="index.php">Home</a></p>
    ...
    <?php
        if(isset($_POST['searchbtn']) && isset($_POST['find'])){
            ...
            $sql="SELECT * FROM insurance WHERE iname LIKE '%$search%' OR illness LIKE '%$search%' OR hname LIKE '%$search%'";
            $result=$mysqli->query($sql);
            $queryResult = mysqli_num_rows($result);
            echo "They are ".$queryResult. " results";
            if($queryResult>0){
                while($row=mysqli_fetch_assoc($result)){
                    echo "<br><div class='treatmentbox'>
                    <h3>".$row['iname']."</h3>
                    <p><span>Illness</span>:".$row['illness']."</p>
                    <p><span>Hospital</span>:".$row['hname']."</p>
                    <p><span>Package</span>:".$row['package']."</p>
                    <p><span>Coverage</span>:".$row['coverage']."</p><br><br>
                </div>";
                }
            }
        }
